  ...
  saveJournal(entry) {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        console.warn('LocalStorage limit reached. Saving journal without heavy images in local storage:', quotaError);
        // Fallback: Strip heavy images array for LocalStorage to preserve text data
        const lightweightUpdated = updated.map((item) => ({
          ...item,
          images: item.images ? item.images.slice(0, 1) : [],
        }));
        try {
          localStorage.setItem(STORAGE_KEYS.JOURNALS, JSON.stringify(lightweightUpdated));
        } catch (e2) {
          console.error('Failed to save to local storage:', e2);
        }
      }
    }
    return newEntry;
